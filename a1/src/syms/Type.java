...
public abstract class Type {
    ...
    /** Calculate the least type that is greater than or equal to both t1 and t2 */
    public static Type join(Type t1, Type t2) {
        if (!t1.equals(t2) && t1 instanceof ReferenceType r1) {
            t1 = r1.getBaseType();
        }
        if (!t2.equals(t1) && t2 instanceof ReferenceType r2) {
            t2 = r2.getBaseType();
        }
        if (!t1.equals(t2) && t1 instanceof SubrangeType s1) {
            t1 = s1.getBaseType();
        }
        if (!t2.equals(t1) && t2 instanceof SubrangeType s2) {
            t2 = s2.getBaseType();
        }
        if (t1.equals(t2)) {
            return t1;
    ...
    /** Calculate the least type that is greater than or equal to the given types */
    public static Type join(List<Type> types) {
        if(types.isEmpty()) {
            return Type.ERROR_TYPE;
        }else{
            Type joinType = types.get(0);
            for(int i = 1; i < types.size(); i++){
                joinType = join(joinType, types.get(i));
            }
            return joinType;
        }
    }
    ...
    public static class IdRefType extends Type {
        ...
        public String toString() {
            return "IdRef(" + id + ")" +
                    (resolved ? " resolved to " + realType : " is unresolved");
        }
